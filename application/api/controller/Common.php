...
class Common extends Controller
{
    ...
    private function SiteStstusCheck()
    {
        if (MyC('home_site_state') != 1) {
            exit(json_encode(DataReturn(MyC('home_site_close_reason', '网站维护中...'), -10000), JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    {
        if (empty($this->user)) {
            exit(json_encode(DataReturn('登录失效，请重新登录', -400), JSON_UNESCAPED_UNICODE));
        }
    }
    ...
    {
        exit(json_encode(DataReturn($name . ' 非法访问', -1000), JSON_UNESCAPED_UNICODE));
    }
}
